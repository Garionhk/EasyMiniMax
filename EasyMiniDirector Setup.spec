...
"""EasyMiniDirector Setup - the installer, with the program inside it.

Built by "Build EXE.bat", *after* EasyMiniDirector.exe, because it carries that exe
...
APP_NAME = 'EasyMiniDirector Setup'
APP_EXE = os.path.join(SPECPATH, 'dist', 'EasyMiniDirector.exe')
...
if not os.path.isfile(APP_EXE):
    raise SystemExit(
        "dist\\EasyMiniDirector.exe is not there, so the installer would have "
        "nothing to install.\n"
...
if app_built < newest:
    raise SystemExit(
        "dist\\EasyMiniDirector.exe is older than the code it was built from "
...
# NOTE: EasyMiniDirector.exe is in datas above and must NOT be dropped from
...
exe = EXE(
    ...
    version=version_file(os.path.join(SPECPATH, 'build'), APP_NAME,
                         'Installs EasyMiniDirector into your EasyAI folder',
                         VERSION),
)
