...
#: A face that can draw Chinese, named after the Latin one in every
#: font-family rule.
#:
#: Qt does fall back on its own when a glyph is missing, but it chooses per
#: glyph and the result is a line of Latin in one face and Chinese in another,
#: at visibly different weights. Naming the fallback keeps a translated sentence
#: looking like one sentence. Windows has had JhengHei since 7; the rest are for
#: a machine that has had fonts removed.
...
def load_fonts() -> tuple[str, str]:
    ...
    CJK_FONT = first("Microsoft JhengHei UI", "Microsoft JhengHei",
                     "Noto Sans TC", "PMingLiU", "Microsoft YaHei UI",
                     "Microsoft YaHei")
...
QLabel#Mono {{
    font-family: "{MONO_FONT}", "{CJK_FONT}";
    font-size: 11px;
    color: {MUTED};
}}
QLabel#MonoAccent {{
...
/* A button holding one glyph and nothing else - the X that removes a shot or
   takes an item out of the queue. The ordinary 14px of side padding is wider
   than the button itself at this size, so without this the glyph is squeezed
   out and the button renders as an empty square. */
QPushButton#Icon {{
    padding: 0;
    font-size: 13px;
}}
QPushButton#Icon:hover {{ color: {BAD}; border-color: {BAD}; }}
...
#: Built by tools/make_icons.py from the app icon. Ships with the program.
